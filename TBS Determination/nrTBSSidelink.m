function TBS = nrTBSSidelink(PSSCHparams,SCI1params,availableREs,SCI2_REs)
...
% allocated for the 2nd-stage SCI, the TB size, TBS, is determined
    ...
    if PSSCHparams.DMRSLength == 2
        N_RE_DMRS = 12;
    elseif PSSCHparams.DMRSLength == 3
        N_RE_DMRS = 18;
    else
        N_RE_DMRS = 24;
    end
    ...
    N_RE = N_prime_RE*PSSCHparams.SubchannelSize*PSSCHparams.NumSubchannels...
        - SCI1params.TimeResourcePSCCH*SCI1params.FreqResourcePSCCH*12;   % see clause 8.1.3.2 TS 38.214

    % Compensate for the error in the standard: wrong determination of the
    % number of REs when the PSSCH-DMRS overlap with the PSCCH
    if PSSCHparams.DMRSLength > 2
        N_RE = N_RE + 6*SCI1params.FreqResourcePSCCH;
    end
    ...
    R = PSSCHparams.TargetCodeRate;
    Qm = PSSCHparams.ModulationOrder;
    v = PSSCHparams.NumLayers;   
